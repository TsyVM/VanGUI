// VanGUI: Renderer Backend for Metal
...
//  2026-03-19: Fixed issue in VanGui_ImplMetal_RenderDrawData() if ImTextureID_Invalid is defined to be != 0, which became the default since 2026-03-12. (#9295, #9310)
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-06-11: Added support for VanGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed VanGui_ImplMetal_CreateFontsTexture() and VanGui_ImplMetal_DestroyFontsTexture().
...
//  2021-12-30: Metal: Added Metal C++ support. Enable with '#define VANGUI_IMPL_METAL_CPP' in your imconfig.h file.
//  2021-08-24: Metal: Fixed a crash when clipping rect larger than framebuffer is submitted. (#4464)
//  2021-05-19: Metal: Replaced direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a requirement)
//  2021-02-18: Metal: Change blending equation to preserve alpha in output buffer.
//  2021-01-25: Metal: Fixed texture storage mode when building on Mac Catalyst.
//  2019-05-29: Metal: Added support for large mesh (64K+ vertices), enable VanGuiBackendFlags_RendererHasVtxOffset flag.
...
struct VanGui_ImplMetal_Data
{
    MetalContext*               SharedMetalContext;
    id<MTLRenderCommandEncoder> RenderCommandEncoder;

    VanGui_ImplMetal_Data()      { memset((void*)this, 0, sizeof(*this)); }
};

static VanGui_ImplMetal_Data*    VanGui_ImplMetal_GetBackendData()    { return VanGui::GetCurrentContext() ? (VanGui_ImplMetal_Data*)VanGui::GetIO().BackendRendererUserData : nullptr; }
static void                     VanGui_ImplMetal_DestroyBackendData(){ IM_DELETE(VanGui_ImplMetal_GetBackendData()); }

static inline CFTimeInterval    GetMachAbsoluteTimeInSeconds()      { return (CFTimeInterval)(double)(clock_gettime_nsec_np(CLOCK_UPTIME_RAW) / 1e9); }

#ifdef VANGUI_IMPL_METAL_CPP

#pragma mark - VanGUI Metal C++ Backend API

bool VanGui_ImplMetal_Init(MTL::Device* device)
{
    return VanGui_ImplMetal_Init((__bridge id<MTLDevice>)(device));
}

void VanGui_ImplMetal_NewFrame(MTL::RenderPassDescriptor* renderPassDescriptor)
{
    VanGui_ImplMetal_NewFrame((__bridge MTLRenderPassDescriptor*)(renderPassDescriptor));
}

void VanGui_ImplMetal_RenderDrawData(ImDrawData* draw_data,
                                    MTL::CommandBuffer* commandBuffer,
                                    MTL::RenderCommandEncoder* commandEncoder)
{
    VanGui_ImplMetal_RenderDrawData(draw_data,
                                   (__bridge id<MTLCommandBuffer>)(commandBuffer),
                                   (__bridge id<MTLRenderCommandEncoder>)(commandEncoder));

}

bool VanGui_ImplMetal_CreateDeviceObjects(MTL::Device* device)
{
    return VanGui_ImplMetal_CreateDeviceObjects((__bridge id<MTLDevice>)(device));
}

#endif // #ifdef VANGUI_IMPL_METAL_CPP

#pragma mark - VanGUI Metal Backend API

void VanGui_ImplMetal_NewFrame(MTLRenderPassDescriptor* renderPassDescriptor)
{
    VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData();
    IM_ASSERT(bd != nil && "Context or backend not initialized! Did you call VanGui_ImplMetal_Init()?");
#ifdef VANGUI_IMPL_METAL_CPP
    ...
    if (bd->SharedMetalContext.depthStencilState == nil)
        VanGui_ImplMetal_CreateDeviceObjects(bd->SharedMetalContext.device);
}

static void VanGui_ImplMetal_SetupRenderState(ImDrawData* draw_data, id<MTLCommandBuffer> commandBuffer,
    ...
    MetalBuffer* vertexBuffer, size_t vertexBufferOffset)
{
    ...
    // Our visible vangui space lies from draw_data->DisplayPos (top left) to
...
// Draw callbacks
static void VanGui_ImplMetal_DrawCallback_ResetRenderState(const ImDrawList*, const ImDrawCmd*)  {} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to implement this way.
static void VanGui_ImplMetal_DrawCallback_SetSamplerLinear(const ImDrawList*, const ImDrawCmd*)  { VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData(); [bd->RenderCommandEncoder setFragmentSamplerState:bd->SharedMetalContext.samplerStateLinear atIndex:0]; }
static void VanGui_ImplMetal_DrawCallback_SetSamplerNearest(const ImDrawList*, const ImDrawCmd*) { VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData(); [bd->RenderCommandEncoder setFragmentSamplerState:bd->SharedMetalContext.samplerStateNearest atIndex:0]; }

// Metal Render function.
void VanGui_ImplMetal_RenderDrawData(ImDrawData* draw_data, id<MTLCommandBuffer> commandBuffer, id<MTLRenderCommandEncoder> commandEncoder)
{
    VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData();
    ...
    // (This almost always points to VanGui::GetPlatformIO().Textures[] but is part of ImDrawData to allow overriding or disabling texture updates).
    if (draw_data->Textures != nullptr)
        for (ImTextureData* tex : *draw_data->Textures)
            if (tex->Status != ImTextureStatus_OK)
                VanGui_ImplMetal_UpdateTexture(tex);
    ...
    VanGui_ImplMetal_SetupRenderState(draw_data, commandBuffer, commandEncoder, renderPipelineState, vertexBuffer, 0);
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                if (pcmd->UserCallback == VanGui_ImplMetal_DrawCallback_ResetRenderState)
                    VanGui_ImplMetal_SetupRenderState(draw_data, commandBuffer, commandEncoder, renderPipelineState, vertexBuffer, vertexBufferOffset);
                else
...
static void VanGui_ImplMetal_DestroyTexture(ImTextureData* tex)
{
...
void VanGui_ImplMetal_UpdateTexture(ImTextureData* tex)
{
    VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData();
    ...
    {
        VanGui_ImplMetal_DestroyTexture(tex);
    }
}

bool VanGui_ImplMetal_CreateDeviceObjects(id<MTLDevice> device)
{
    VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData();
...
void VanGui_ImplMetal_DestroyDeviceObjects()
{
    VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData();

    // Destroy all textures
    for (ImTextureData* tex : VanGui::GetPlatformIO().Textures)
        if (tex->RefCount == 1)
            VanGui_ImplMetal_DestroyTexture(tex);
...
bool VanGui_ImplMetal_Init(id<MTLDevice> device)
{
    VanGuiIO& io = VanGui::GetIO();
    VANGUI_CHECKVERSION();
    IM_ASSERT(io.BackendRendererUserData == nullptr && "Already initialized a renderer backend!");

    VanGui_ImplMetal_Data* bd = IM_NEW(VanGui_ImplMetal_Data)();
    io.BackendRendererUserData = (void*)bd;
    io.BackendRendererName = "vangui_impl_metal";
    io.BackendFlags |= VanGuiBackendFlags_RendererHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.
    ...
    platform_io.DrawCallback_ResetRenderState = VanGui_ImplMetal_DrawCallback_ResetRenderState;
    platform_io.DrawCallback_SetSamplerLinear = VanGui_ImplMetal_DrawCallback_SetSamplerLinear;
    platform_io.DrawCallback_SetSamplerNearest = VanGui_ImplMetal_DrawCallback_SetSamplerNearest;
...
void VanGui_ImplMetal_Shutdown()
{
    VanGui_ImplMetal_Data* bd = VanGui_ImplMetal_GetBackendData();
    IM_UNUSED(bd);
    ...
    VanGui_ImplMetal_DestroyDeviceObjects();
    VanGui_ImplMetal_DestroyBackendData();
